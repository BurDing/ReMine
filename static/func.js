...
//var node_types = ["$Cells", "$Cells.Cellular Structures",
//    "$Carbohydrates", "$Proteins", "$Bacteria", "$Diseases",
//    "$Chemicals"]
...
/* .......................................................................... */
  ...
  var force = d3.layout.force()
    .charge(-320)
    .size( [WIDTH, HEIGHT] )
    .linkDistance( function(d,idx) { return d.weight; } )
    //.gravity(0.05)
    .start();
  ...
  */
  function getMovieInfo( n, nodeArray, pmidArray ) {
    console.log( "INFO", n );
    info = '<div id="entity_title">';
    info += '<div class=t style="float: right"><br/>' + n.title + '</div>';
    info +=
    '<img src="img/close.png" class="action" style="right: 0px; top: 0px;" title="close panel" onClick="toggleDiv(\'movieInfo\');"/>' +
    '<img src="img/target-32.png" class="action" style="top: -7px;" title="center graph on movie" onclick="selectMovie('+n.index+',true);"/>';

    info += '<br/></div><div style="clear: both;">';
    if( n.type )
      info += '<div class=f><span class=l>Type</span>: <span class=g>'
           + n.type + '</span></div>';
    if( n.pmid ) {
      info += '<div class=f><span class=l>Related Documents</span>:<br/>';
      n.pmid.forEach( function(idx) {
        myurl = pmidArray[idx].url;
        title = pmidArray[idx].title;
        info += '<span class="docuemnt_title" maxlength="20"><a target="_blank" href=https://www.ncbi.nlm.nih.gov/pubmed/' + myurl + ' >' + title + '</a></span>';
      });
      info += '</div>';
    }
    if( n.links ) {
  ...
  function getEdgeInfo( e, nodeArray, pmidArray )
  {
    info = '<div id="cover">';
    src = nodeArray[e.source.id]
    tar = nodeArray[e.target.id]
    title = src.label + ' -- ' + tar.label

    if( e.cover )
      info += '<img class="cover" height="300" src="' + e.cover + '" title="' + title + '"/>';
    else
      info += '<div class=t style="float: right"><br/>' + title + '</div>';
    info +=
    '<img src="img/close.png" class="action" style="right: 0px; top: 0px;" title="close panel" onClick="toggleDiv(\'movieInfo\');"/>' +
    '<img src="img/target-32.png" class="action" style="top: -7px;" title="center graph on movie" onclick="selectMovie('+src.index+',true);"/>';

    info += '<br/></div><div style="clear: both;">'

    if( e.pmid ) {
      info += '<div class=f><span class=l>Related Documents</span>:<br/>';
      e.pmid.forEach( function(idx) {
        myurl = pmidArray[idx].url;
        title = pmidArray[idx].title;
        info += '<span class="docuemnt_title" maxlength="20"><a target="_blank" href=https://www.ncbi.nlm.nih.gov/pubmed/' + myurl + ' >' + title + '</a></span>';
      });
    ...
    info += '<div class=f><span class=l>Related to</span>: ';
    info += '[<a href="javascript:void(0);" onclick="selectMovie('
            + src.index + ',true);">' + src.label + '</a>]';
    info += '[<a href="javascript:void(0);" onclick="selectMovie('
            + tar.index + ',true);">' + tar.label + '</a>]';
    info += '</div>';
    return info;
  }
  ...
  d3.json(
    ...
    var graphLinks = networkGraph.append('svg:g').attr('class','grp gLinks')
      ...
      .attr("class", "link")
      .on("click", function(d) { showEdge(d); } )
      .on("mouseover", function(d) { highlightGraphEdge(d,true,this);  } )
      .on("mouseout",  function(d) { highlightGraphEdge(d,false,this); } );
    ...
    var graphNodes = networkGraph.append('svg:g').attr('class','grp gNodes')
      ...
      .style("fill",function(d){return colors(data.node_types.indexOf(d.type));})
    ...
    function highlightSingleNode(node, on) {
    ...
    function highlightGraphEdge(e, on) {
      if( on ) {
        if ( activeMovie !== undefined ) {
        ...
        if ( activeEdge !== undefined) {
          console.log("..clear: ",activeEdge);
          highlightGraphEdge( activeEdge, false );
          console.log("..cleared: ",activeEdge);
        }
      }
      src = nodeArray[e.source.id]
      tar = nodeArray[e.target.id]

      highlightSingleNode(src, on);
      highlightSingleNode(tar, on);

      // set the value for the current active movie
      activeEdge = on ? e : undefined;
      console.log("SHOWNODE finished: [" + e + "] = "+on );
    }
    ...
    */
    ...
    function showMoviePanel( node ) {
      ...
      movieInfoDiv
    	.html( getMovieInfo(node, nodeArray, pmidArray) )
    	.attr("class","panel_on");
    }

    function showEdge( edge ) {
      movieInfoDiv
      .html( getEdgeInfo(edge, nodeArray, pmidArray) )
      .attr("class","panel_on");
    }
